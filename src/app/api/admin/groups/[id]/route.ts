...
export async function PUT(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  await prisma.$transaction(async (tx) => {
    await tx.employeeGroup.update({ where: { id }, data: { name: name.slice(0, 120) } });
    if (employeeIds) {
      await tx.employeeGroupMember.deleteMany({ where: { groupId: id } });
      if (employeeIds.length > 0) {
        await tx.employeeGroupMember.createMany({
          data: employeeIds.map((employeeId) => ({ groupId: id, employeeId: String(employeeId) })),
        });
      }
    }
  });
...
export async function DELETE(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const g = await prisma.employeeGroup.findUnique({
    ...
    select: {
      name: true,
      _count: { select: { forms: true, notices: true, loginAnnouncements: true } },
    },
  });
  if (!g) return NextResponse.json({ error: "그룹을 찾을 수 없습니다." }, { status: 404 });
  const used = g._count.forms + g._count.notices + g._count.loginAnnouncements;
  if (used > 0) {
    return NextResponse.json(
      { error: "이 그룹을 사용 중인 양식·공지·팝업이 있어 삭제할 수 없습니다." },
      { status: 400 },
    );
  }
