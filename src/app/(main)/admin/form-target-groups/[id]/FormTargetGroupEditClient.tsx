...
export default function FormTargetGroupEditClient({
  groupId,
  initialName,
  initialMembers,
}: {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    setSaving(true);
    const res = await fetch(`/api/admin/form-target-groups/${groupId}`, {
      method: "PUT",
    ...
    router.push("/admin/form-target-groups");
    router.refresh();
  }
  ...
  return (
    <form onSubmit={handleSave} className="space-y-6 max-w-3xl">
      <div>
        <Link href="/admin/form-target-groups" className="text-sm text-gray-500 hover:text-gray-700">
          ← 대상 그룹 목록
        </Link>
      </div>
