...
export default function FormTargetGroupsClient({ initialGroups }: { initialGroups: GroupRow[] }) {
  ...
  async function loadPicker() {
    if (employees) return;
    const res = await fetch("/api/admin/form-target-groups/picker-employees");
    if (!res.ok) return;
  ...
  async function handleDelete(id: string, name: string, formCount: number) {
    if (formCount > 0) {
      alert(`「${name}」을(를) 사용 중인 양식이 있어 삭제할 수 없습니다.`);
      return;
    }
    if (!confirm(`그룹「${name}」을(를) 삭제할까요?`)) return;
    const res = await fetch(`/api/admin/form-target-groups/${id}`, { method: "DELETE" });
    const data = await res.json().catch(() => ({}));
    if (!res.ok) {
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-gray-600">
          유동 양식에서 &quot;지정 그룹&quot;으로 선택하면, 접근·알림톡 대상이 이 목록과 동일하게 적용됩니다.
        </p>
        <button
          type="button"
          ...
          >
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-gray-500 mt-0.5">
                사원 {g._count.members}명 · 연결된 양식 {g._count.forms}개
              </p>
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              <Link
                href={`/admin/form-target-groups/${g.id}`}
