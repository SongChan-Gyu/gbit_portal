#!/usr/bin/env node
/**
 * 다이렉트센드 카카오 알림톡 발송 예시 (PHP directsend_kakao_allim_php_api.php 와 동일 API)
 *
 * 사용:
 ...
 *   node scripts/directsend/alimtalk-example.mjs
 *
 * 문서의 note1~note5 는 템플릿 치환용(미사용 시 빈 문자열 가능). 번호만내려면 mobile 만 채우면 됨.
 */
