 ...
 *   스탬프·하프데이·생일반차·근속·힐링데이 등은 1단(PM·관리자 본인은 자동).
 *
...
async function seedLeaveTypes() {
  ...
  const types = [
    ...
    ["DUTY_DEPT", "직무부서휴가", 1, false, 1, null, null, false, null, false, false, false, "귀속연도", null, "#8b5cf6", 23],
  ...
  function allocationSourceForCode(code: string): string | null {
    ...
    if (code === "DUTY_DEPT") return "DUTY_DEPT";
    return null;
  }
  ...
  function displayHintForCode(code: string): string | null {
    ...
    if (code === "HEALING_DAY_HALF_REPLACE") return "하프데이 유무와 무관 · 월 1회 · 0일 · 승인 시 하프데이 자동 취소";
...
async function seedAllocationSourceConfigs() {
  ...
  const configs: {
    ...
    { sourceCode: "HOLIDAY_EXT",  label: "연휴연장", sortOrder: 4, defaultDays: 1,   note: "전원 1일" },
