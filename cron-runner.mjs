...
// ─────────────────────────────────────────────────────
// 3. 매일 오전 0시 20분 — 제주 숙소 입금 확인 기한 초과 자동 취소
// ─────────────────────────────────────────────────────
cron.schedule("20 0 * * *", async () => {
  log("▶ 제주 입금 기한 자동취소 시작");
  const result = await callApi("/api/cron/jeju-deposit-deadline", {});
  if (result.error) {
    log(`  ❌ 실패: ${result.error}`);
  } else {
    log(
      `  ✅ 대상 ${result.candidates ?? 0}건, 처리 ${(result.cancelled ?? []).length}건, 오류 ${(result.errors ?? []).length}건`,
    );
  }
}, { timezone: "Asia/Seoul" });
